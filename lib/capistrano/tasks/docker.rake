namespace :docker do
  task :stop  => %w(proxy:stop servers:stop public:stop)
    ...
    task :setup do
      on roles :app do
        within File.join(current_path, fetch(:public)) do
          execute :docker, "create --name #{fetch :public} -v #{File.join(current_path, 'public')}:/home/web/app/public dtheus/rails:4.1.7 /bin/true"
        end
      end
    end
